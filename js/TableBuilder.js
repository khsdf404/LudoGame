...
const isColor = (i, j) => {
   ...
   for(let k = 0; k < 6; k++) {
      if (i == green[k][0] && j == green[k][1])
         return k == 0 && 'greenbg start-cell' || i == 0 && 'greenbg finish-cell' || 'greenbg finish-path';
      if (i == blue[k][0] && j == blue[k][1])
         return k == 0 && 'bluebg start-cell' || j == 0 && 'bluebg finish-cell' || 'bluebg finish-path';
      if (i == red[k][0] && j == red[k][1])
         return k == 0 && 'redbg start-cell' || i == 13 && 'redbg finish-cell' || 'redbg finish-path';
      if (i == yellow[k][0] && j == yellow[k][1])
         return k == 0 && 'yellowbg start-cell' || j == 7 && 'yellowbg finish-cell' || 'yellowbg finish-path';
   }
   return null;
}
function setTable () {
